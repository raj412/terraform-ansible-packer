...
def create_alert(alertname,description,summary,message,alert_level="warning"):
    log.set_level('INFO')
    log.write(level=3,message="Starting create_alert")

    alertFile= alertPath + alertname + ".alt"
    log.write(level=3,message="Alert file = %s" %(alertFile))
    ...
    with open(alertFile,"w+") as af:
        af.write(alertJson)
    log.write(level=3, message="Alert File %s was written" %(alertFile))

def delete_alert(alertname):
    log.set_level('INFO')
    log.write(level=3,message="Starting delete_alert")
    alertFile= alertPath + alertname + ".alt"
    log.write(level=3,message="Alert file = %s" %(alertFile))
    # check to see if there is an alert file already
    fileList= glob.glob(alertFile)
    if len(fileList) != 0:
        log.write(level=3,message="removing Alert file")
        os.remove(alertFile)